...
import pandas as pd
import user_blast
import user_circ_seq
import user_feature_N
import user_feature_O
import user_feature_J
import user_double
...
df_alis = user_blast.exactReads(info_file)
df_list = user_blast.cutDatafram(df_alis)
pool = multiprocessing.Pool(len(df_list))
print('Alignment reads begining')
for dataframe in df_list:
    pool.apply_async(user_blast.getCircInformation,
                    (dataframe, alignment_res, ))
pool.close()
pool.join()
print('Alignment reads ending')
junction_filename, circ_filename = user_circ_seq.get_seq(
    alignment_res, genome_file)
...
orf_p = user_double.double(circ_filename)
orf_f = orf_p + '.gb'
os.system("ugene find-orfs --in=%s --out=%s" % (orf_p, orf_f))  # invoke the ugene

df_N, df_J, df_O = user_feature_N.extractNuc(circ_filename),\
                   user_feature_J.feature_J(junction_filename),\
                   user_feature_O.get_orf(orf_f)
