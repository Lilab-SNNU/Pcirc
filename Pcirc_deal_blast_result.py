...
def exactReads(filename):
    ...
    df = df.drop_duplicates(subset=[0, 1, 3, 6, 7], keep=False)  #去除完全重复的全长比对reads
    return df
...
def getCircInformation(df, file):
    ...
    circ_start_end = []
    # with tqdm(total=len(set_name), bar_format='{l_bar}{l_bar}|{n_fmt}/{total_fmt}{rate_fmt}', ncols=80) as pbar:
